...
describe("checkCoupon", () => {
  it("takes a percentage off the subtotal", async () => {
    ...
    const result = await checkCoupon("SAVE10", 200);

    expect(result).toEqual({ ok: true, id: "c1", code: "SAVE10", discount: 50 });
  });

  it("takes a fixed amount off", async () => {
    findUnique.mockResolvedValue(coupon({ percentOff: null, amountOff: 15 }));

    const result = await checkCoupon("SAVE10", 200);

    expect(result).toMatchObject({ ok: true, discount: 15 });
  });

  it("upper-cases and trims the code before looking it up", async () => {
    findUnique.mockResolvedValue(coupon());

    await checkCoupon("  save10  ", 100);

    expect(findUnique).toHaveBeenCalledWith({ where: { code: "SAVE10" } });
  });

  it("rejects an empty code without querying", async () => {
    const result = await checkCoupon("   ", 100);
  ...
  it("rejects a code that does not exist", async () => {
    ...
    expect(await checkCoupon("NOPE", 100)).toEqual({ ok: false, reason: "not-found" });
  });
  ...
  it("rejects a subtotal below the minimum", async () => {
    findUnique.mockResolvedValue(coupon({ minOrderTotal: 100 }));

    expect(await checkCoupon("SAVE10", 99.99)).toEqual({ ok: false, reason: "min-total" });
  });
  ...
  it("never discounts more than the subtotal", async () => {
    findUnique.mockResolvedValue(coupon({ percentOff: null, amountOff: 500 }));

    // Otherwise a fixed-amount coupon on a small basket produces a negative
    // total and the shop pays the customer.
    expect(await checkCoupon("SAVE10", 30)).toMatchObject({ ok: true, discount: 30 });
  });

  it("rounds to whole tetri", async () => {
    findUnique.mockResolvedValue(coupon({ percentOff: 33 }));

    // 33% of 10.10 is 3.333 — must not reach the order as a long float.
    expect(await checkCoupon("SAVE10", 10.1)).toMatchObject({ discount: 3.33 });
  });
  ...
  it("treats a coupon with neither percent nor amount as zero, not NaN", async () => {
    ...
    expect(await checkCoupon("SAVE10", 100)).toMatchObject({ ok: true, discount: 0 });
  });
});
