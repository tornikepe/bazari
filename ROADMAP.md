...
- ~~**Uptime**~~ გაკეთებულია (2026-09-17): UptimeRobot ამოწმებს
  `bazari-tornikepes-projects.vercel.app/api/health`-ს ყოველ 5 წუთში; 503-ზე შენს ელფოსტაზე
  მოვა. დომენის შემდეგ მონიტორის URL შეცვალე.
- **ბექაპები** — ჩვენი არსებობს და აღდგენა შემოწმებულია (`npm run db:backup`,
  `npm run db:restore`). Neon-ის „Restore from history" **6 საათს** ფარავს (შემოწმებულია) — ანუ
  შემთხვევითი წაშლიდან 6 საათში უნდა მიხვდე. ყოველი მიგრაციის წინ `npm run db:backup`. თუ
  ყოველდღიური ავტომატური ბექაპი დაგჭირდება — ერთი cron-ია, მითხარი.
...
3. ~~§4 — uptime, Neon-ის retention~~ გაკეთებულია
4. **§5** — ტელეფონზე ცოცხალი შემოწმება (vercel.app-ზე) — **შემდეგი**
